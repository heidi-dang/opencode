...
install_from_local_repo() {
    ...
    # Build the project
    cd "$REPO_ROOT"
    print_message info "${MUTED}Running bun install at root...${NC}"
    if ! bun install; then
        print_message error "Failed to run bun install"
        exit 1
    fi
    if ! bun run --cwd packages/opencode build; then
        print_message error "Failed to build the project"
        exit 1
    fi

    print_message info "${MUTED}Building local UI dist...${NC}"
    if ! bun run --cwd packages/app build; then
        print_message error "Failed to build local UI dist"
        exit 1
    fi

    UI_DIST="$REPO_ROOT/packages/app/dist"
    if [ ! -f "$UI_DIST/index.html" ] || [ ! -f "$UI_DIST/opencode-build.json" ]; then
        print_message error "Local UI dist is incomplete at $UI_DIST"
        exit 1
    fi
