...
const description = "Find likely code owners for a symbol or path using git history and the local repo index."
...
export default tool({
  ...
  async execute(args, ctx) {
    ...
    const cmd = ["owners", "--root", ctx.worktree, "--target", args.target]
    ...
    const err = [Buffer.from(py3.stderr).toString("utf8"), Buffer.from(py.stderr).toString("utf8")]
      .filter(Boolean)
      .join("\n")
    throw new Error(`py_find_owners failed\n${err}`)
  },
})
