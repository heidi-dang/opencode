...
const description = "Get related code context for a symbol or path, including file summary, imports/exports, local symbols, and related tests."
...
export default tool({
  ...
  async execute(args, ctx) {
    ...
    const cmd = ["related", "--root", ctx.worktree, "--target", args.target]
    ...
    const err = [Buffer.from(py3.stderr).toString("utf8"), Buffer.from(py.stderr).toString("utf8")]
      .filter(Boolean)
      .join("\n")
    throw new Error(`repo_related_code failed\n${err}`)
  },
})
