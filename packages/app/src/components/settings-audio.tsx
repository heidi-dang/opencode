...
export const SettingsAudio: Component = () => {
  ...
  const list = () => previewList(settings.workflowAudio.pack())
    ...
    <div class="flex flex-col h-full overflow-y-auto no-scrollbar px-4 pb-10 sm:px-10 sm:pb-10">
      ...
      <div class="flex flex-col gap-8 w-full">
        <div class="flex flex-col gap-3 rounded-2xl border border-border-weak-base bg-surface-panel px-4 py-4">
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex flex-col gap-1">
              ...
              <span class="text-12-regular text-text-weak">Open the internal iteration surface for waveform, scores, and version history.</span>
            </div>
            <a href={`/${params.dir}/internal/audio-preview`} class="inline-flex">
              <Button size="small" variant="secondary">Open preview page</Button>
            </a>
          </div>
          ...
          <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px]">
            <div class="flex min-w-0 flex-col gap-2">
              ...
              <span class="text-12-regular text-text-weak">Choose the cue family used across turn, tool, planner, and attention events.</span>
            </div>
            <Select
              ...
              onHighlight={(option) => {
                if (!option) return
                play(previewSrc(option.value), settings.workflowAudio.volume() / 100)
              }}
              onSelect={(option) => {
                if (!option) return
          ...
          <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px]">
            <div class="flex min-w-0 flex-col gap-2">
              ...
              <span class="text-12-regular text-text-weak">Browser preview uses the same workflow-audio level as runtime playback intent.</span>
            </div>
          ...
          <div class="flex flex-wrap items-center gap-4 justify-between">
            <div class="flex min-w-0 flex-1 flex-col gap-1">
              ...
              <span class="text-12-regular text-text-weak">Show the recent workflow-audio queue and playback decisions on screen.</span>
            </div>
            <Switch checked={settings.workflowAudio.debug()} onChange={(checked) => settings.workflowAudio.setDebug(checked)} />
          </div>
        </div>
        ...
        <div class="flex flex-col gap-3 rounded-2xl border border-border-weak-base bg-surface-panel px-4 py-4">
          ...
          <Show
            when={list().length > 0}
            fallback={<div class="text-12-regular text-text-weak">Preview metadata is currently available for Minimal Pro.</div>}
          >
