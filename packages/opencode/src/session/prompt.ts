...
export namespace SessionPrompt {
  ...
  export function buildPlanReminder(input: { plan: string; exists: boolean }) {
...
Goal: Gain a comprehensive understanding of the user's request by reading through code and identifying the best safe approach. Critical: In this phase you should only use the explore subagent type.
...
3. Make the best reasonable assumptions and proceed. Record those assumptions clearly in the plan instead of asking routine preference questions.

4. Use the question tool only if you are blocked by one of these cases:
   - missing required user-specific information
   - safety, destructive, or irreversible operations
   - true ambiguity with materially different outcomes
...
3. Ask the user a question only if you remain blocked by one of the listed exceptional cases
...
- Document any reasonable assumptions you made so execution can proceed without further user input

### Phase 5: Call plan_exit tool
At the end of your turn, once your final plan file is ready, you should call plan_exit to indicate that planning is complete.
This is the normal way to finish planning. Only end by asking the user a question when you are truly blocked by one of the exceptional cases above.

**Important:** Use question tool only for blocked cases, and use plan_exit to request plan approval. Do NOT use question tool to ask routine preference questions or "Is this plan okay?".
</system-reminder>`
  }
  ...
  async function insertReminders(input: { messages: MessageV2.WithParts[]; agent: Agent.Info; session: Session.Info }) {
    ...
    if (input.agent.name === "plan" && assistantMessage?.info.agent !== "plan") {
      ...
      const part = await Session.updatePart({
        ...
        sessionID: userMessage.info.sessionID,
        type: "text",
        text: buildPlanReminder({ plan, exists }),
        synthetic: true,
      })
      userMessage.parts.push(part)
      return input.messages
