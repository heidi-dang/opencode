...
export class UiSourceResolver {
  ...
  static async resolve(opts: {
    ...
    if (uiDir) {
      const fullPath = path.resolve(uiDir)
      if (await this.isValidDist(fullPath)) {
        return { 
          source: { type: "dist", path: fullPath }, 
          reason: "env OPENCODE_UI_DIR"
        }
      }
      this.log.error(`!! Requested env OPENCODE_UI_DIR "${uiDir}" is invalid (missing index.html).`, {
        path: fullPath,
    ...
    const configDevUrl = (config as any)?.uiDevUrl || process.env.OPENCODE_UI_DEV_URL
    if (configDevUrl) {
    ...
    if ((config as any)?.uiDist) {
      const fullPath = path.resolve((config as any).uiDist)
      if (await this.isValidDist(fullPath)) {
        return { 
          source: { type: "dist", path: fullPath }, 
          reason: "explicit uiDist"
      ...
      this.log.warn(`Config server.uiDist "${(config as any).uiDist}" is invalid (missing index.html).`, {
        path: fullPath,
        action: "Falling back to next available source"
      })
    }
