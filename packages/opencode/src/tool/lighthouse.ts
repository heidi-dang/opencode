...
export const LighthouseTool = Tool.define("lighthouse_audit", async (ctx) => {
  return {
    ...
    async execute(params: z.infer<typeof parameters>, ctx) {
      ...
      // Specialized performance wrapper executing via npx lh-cli
      const cmd = `npx --yes lighthouse ${params.url} --quiet --output=json --output-path=./lh-report.json --chrome-flags="--headless"`
      ...
      return new Promise<{ title: string; output: string; metadata: any }>((resolve) => {
        ...
        proc.on("close", async (code) => {
          try {
            ...
            const reportPath = path.join(cwd, "lh-report.json")
            ...
            resolve({ title: "Lighthouse Trace", output: `Lighthouse Audit Complete for ${params.url}:\n- LCP: ${lcp}\n- CLS: ${cls}\n- FCP: ${fcp}\n- TTI: ${tti}\n\nReview these metrics to optimize web lifecycles.`, metadata: {} })
          } catch (e) {
            resolve({ title: "Lighthouse Trace", output: `Lighthouse failed or report unreadable. Stderr: ${stderr.slice(0, 500)}`, metadata: {} })
          }
        })
      })
    },
