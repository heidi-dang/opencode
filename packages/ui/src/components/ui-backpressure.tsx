...
class UIBackpressureController {
  ...
  private getPressureLevel(): 'none' | 'low' | 'medium' | 'high' | 'critical' {
...
class StreamConsumerWithBackpressure {
  ...
  constructor(options: Partial<StreamConsumerOptions> = {}) {
    ...
    this.backpressureController = new UIBackpressureController(
      this.options.backpressureThresholds,
      {
        enableAdaptive: true,
        enableQueueLimit: true,
        enableTimeLimit: true,
...
export function BackpressureIndicator(): JSX.Element {
  ...
  const getIndicatorColor = () => {
    ...
    switch (level) {
      case 'none': return 'var(--color-success)'
