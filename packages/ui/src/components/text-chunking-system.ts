...
class TextChunker {
  ...
  private hasBoundary(content: string, boundaries: string[]): boolean {
    ...
    return boundaries.some((boundary) => content.endsWith(boundary) || content.slice(-10).includes(boundary))
  }
...
class SmartContentTypeDetector {
  ...
  private isCode(content: string, context: string): boolean {
    ...
    return codeIndicators.some((pattern) => pattern.test(content)) || context.includes("```") || content.includes("```")
  }
...
class ChunkingFrameBatcher {
  ...
  addStreamingText(messageId: string, partId: string, text: string, context: string = ""): void {
  ...
  initialize(): void {
    window.addEventListener("textChunk", (event: Event) => {
      const customEvent = event as CustomEvent
      const { key, content, type, priority, timestamp } = customEvent.detail

      // Parse key back to messageId and partId
      const [messageId, partId] = key.split(":")

      // Add to frame batcher with appropriate priority
      this.frameBatcher.addDelta(
        messageId,
        partId,
        {
          type: "text",
