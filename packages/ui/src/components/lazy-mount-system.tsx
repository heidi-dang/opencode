...
export function LazyMount(props: LazyComponentProps): JSX.Element {
  ...
  createEffect(() => {
    ...
    const handleIntersection = () => {
      ...
      if (props.defer) {
        ...
        lazyMountManager.setDeferredTimer(
          componentId,
          () => {
            setShouldMount(true)
            setIsMounted(true)
  ...
  return (
    <div ref={elementRef} class="lazy-mount-container" style={{ contain: "layout" }}>
      <Show when={shouldMount()} fallback={props.placeholder ?? defaultPlaceholder}>
        {props.children}
      </Show>
    </div>
  )
...
export function LazyComputation<T>(props: LazyComputationProps<T>): JSX.Element {
  ...
  const defaultPlaceholder = (
    <div
      class="lazy-computation-placeholder"
      style={{
        "min-height": "60px",
        "background-color": "var(--color-bg-secondary)",
        border: "1px dashed var(--color-border-primary)",
  ...
  createEffect(() => {
    ...
    const handleIntersection = async () => {
      ...
      try {
        ...
        setResult(computationResult as any)
        lazyMountManager.markMounted(componentId)
      } catch (err) {
  ...
  return (
    <div ref={elementRef} class="lazy-computation-container" style={{ contain: "layout" }}>
      <Show when={resultValue !== undefined}>{props.children(resultValue!)}</Show>

      <Show when={errorValue}>
        <div
          class="computation-error"
          style={{
            padding: "8px",
      ...
      <Show when={isComputingValue}>{props.placeholder ?? defaultPlaceholder}</Show>
...
export function LazyList<T>(props: LazyListProps<T>): JSX.Element {
  ...
  return (
    <div
      ...
      style={{
        height: `${props.containerHeight ?? 400}px`,
        overflow: "auto",
        position: "relative",
      }}
...
export function DebouncedLazy(props: DebouncedLazyProps): JSX.Element {
  ...
  return (
    <div ref={elementRef} class="debounced-lazy-container" style={{ contain: "layout" }}>
      <Show when={shouldShow()} fallback={props.placeholder}>
        {props.children}
      </Show>
    </div>
  )
