...
class MessageVirtualizer {
  ...
  handleScroll(): void {
    ...
    this.state.visibleRange = this.calculateVisibleRange(
    ...
    >
      <div class="message-header">
        ...
        <span class="message-time">
          {new Date(props.message.timestamp || 0).toLocaleTimeString()}
        </span>
      </div>
...
export function VirtualizedHistoryLane(props: VirtualizedHistoryLaneProps): JSX.Element {
  const [virtualizer] = createSignal(() => new MessageVirtualizer())
  ...
  createEffect(() => {
    ...
    if (container) {
      vz.setContainer(container)
      vz.updateTotalItems(messageList.length)
      
      // Generate virtual items
      const items = vz.generateVirtualItems(messageList)
      ...
      >
        ...
        <For each={virtualItems()}>
          {(item) => {
            ...
            return message ? (
              <VirtualMessageItem item={item} message={message} />
            ) : null
          }}
        </For>
      </div>
